...
def classify_iris_heuristic(petal_length: float, petal_width: float) -> str:
    ...
    Examples:
        ...
        'virginica'
    """
    logger.debug("Classifying iris sample", 
    ...
    if petal_length < 0 or petal_width < 0:
        ...
        logger.error("Invalid input validation failed", 
                    error=error_msg,
                    petal_length=petal_length, 
                    petal_width=petal_width)
    ...
    if petal_length > 10 or petal_width > 5:
        ...
        logger.warning("Unrealistic measurements detected", 
                      warning=error_msg,
                      petal_length=petal_length, 
                      petal_width=petal_width)
        raise ValueError(error_msg)
    ...
    if petal_length < 2.0:
        prediction = 'setosa'
        logger.debug("Applied rule 1 - Setosa separation", 
                    rule="petal_length < 2.0",
    ...
    # Rule 2: Versicolor vs Virginica separation (EDA finding: petal_width threshold)
        ...
        logger.debug("Applied rule 2 - Versicolor threshold", 
    ...
    # Rule 3: Default to Virginica for large petal measurements
        ...
        logger.debug("Applied rule 3 - Virginica default", 
                    rule="else (large petals)",
                    prediction=prediction)
        return prediction


def classify_batch(petal_lengths: Union[List[float], np.ndarray[Any, Any]], 
                  petal_widths: Union[List[float], np.ndarray[Any, Any]]) -> List[str]:
    """
    ...
    logger.info("Starting batch classification", 
               sample_count=len(petal_lengths))
