...
def evaluate_model(y_true: np.ndarray[Any, Any], y_pred: np.ndarray[Any, Any], iris_data: Bunch) -> Dict[str, Any]:
    ...
    for species in iris_data.target_names:
        ...
        species_accuracy = accuracy_score([species] * len(species_predictions), 
                                        species_predictions)
    ...
    for idx in misclassified_indices:
        ...
        logger.debug("Misclassification detected",
                    sample_index=idx,
...
def log_performance_summary(results: Dict[str, Any], experiment_name: str) -> None:
    ...
    if algorithm_details:
        ...
        if 'rules' in algorithm_details:
            logger.info("Algorithm rules applied",
                       rules=algorithm_details['rules'])
    
    # Log misclassifications
    if results['misclassifications']:
        logger.info("Misclassifications detected",
                   total_misclassifications=len(results['misclassifications']))
        
        for misc in results['misclassifications']:
            logger.warning("Misclassification detail",
    ...
    if overall_acc >= 0.95:
        ...
        logger.warning("Target accuracy not met",
